    ...
    class Program
    {
        ...
        //U
                        else if (userChoiceString.ToUpper().Equals("U"))
                        {
                            ...
                            {
                                if (!int.TryParse(Console.ReadLine(), out membershipIDToUpdate) || membershipIDToUpdate < 0)
                                {
                                    Console.WriteLine("Invalid input. Please enter a valid positive integer for the membership ID.");
                                    continue;
                                }
                                ...
                                {
                                    Console.Write("Membership ID not found, please enter a valid membership ID: ");
                                }
                        }
                        ...
                        if (membershipTypeInput.ToUpper() == "NONPROFIT")
                        {
                            Console.Write("Is this a military or educational organization? (yes/no): ");
                            string militaryOrEducationalInput = Console.ReadLine();
        ...
        //D             D - Delete an existing membership from the membership list.
            ...
            //          the membership exists, perform the return by subtracting the return amount for the monthly purchase total.
                        ...
                        else if (userChoiceString.ToUpper().Equals("T"))
                        {
                            ...
                            Membership membershipFound = membershipList.Find(m => m.MembershipID == membershipIDForReturn);
                            if (membershipFound != null)
                            {
                                ...
                                while (running2)
                                {
                                    if (decimal.TryParse(Console.ReadLine(), out returnAmount))
                                    {
                                        ...
                                        Console.WriteLine("Return successful. New current monthly purchases amount is: " + membershipFound.CurrentMonthlyPurchases.ToString("C"));
                                        running2 = false;
                                    }
                                    else
                                    {
        ...
        //             "Cash-back reward request for membership xxxxxx in the amount of $yyyy has been made." Then zero out the balance. 
                        ...
                        else if (userChoiceString.ToUpper().Equals("A"))
                        {
                            ...
                            while (running)
                            {
                                ...
                                Membership membershipFound = membershipList.Find(m => m.MembershipID == membershipIDInput);
                                if (membershipFound != null)
                                {
